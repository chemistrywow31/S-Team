...
Every agent must enforce two structural gates: a `## Reasoning` gate that runs before the workflow (think before acting) and a `## Self-Critique` gate that runs after the workflow produces a draft, before submission (challenge before submitting). Both are structural sections in the agent .md file, not instructional sentences — Claude reliably follows structural boundaries.

### Section Ordering in the Agent Template

Every agent .md must order these three sections as: `## Reasoning` → `## Workflow` → `## Self-Critique`. This creates a tight think → act → verify triad. Do not separate them; do not place Skills or Rules sections between them.
...
The Coordinator must additionally run a `## Pre-Dispatch Reasoning` gate before each Task dispatch, with four slots: `### What This Dispatch Must Achieve`, `### Why This Agent`, `### Inputs the Agent Needs`, `### Predicted Failure Modes`.
...
- Agent .md missing `## Reasoning` or `## Self-Critique` section → Violation
...
- Deterministic Tier 1 utilities (single-lookup, zero judgment calls) may use a reduced 2-check Self-Critique (`### Format Check`, `### Input Coverage Check`) and may omit `## Reasoning` when the agent .md states the Tier 1 justification.
