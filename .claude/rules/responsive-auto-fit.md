...
This rule's compliance is enforced by `rules/layout-gate.md`. Visual self-attestation is no longer accepted. Web Developer must:

1. Copy `skills/layout-gate/scripts/auto-fit-snippet.html` verbatim into `index.html` immediately after `Reveal.initialize()`.
2. Run `node .claude/skills/layout-gate/scripts/layout-gate.cjs <output>/index.html` and confirm exit code `0`.
3. Cite `layout-gate-report.json` in the completion summary per `rules/layout-gate.md`.
...
- QA Reviewer approves a presentation where the layout gate has not been run or has not produced `"status": "PASS"` (per `rules/layout-gate.md`) → Violation
