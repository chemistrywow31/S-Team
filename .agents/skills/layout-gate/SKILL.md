...
Replace self-reported "I checked the browser" claims with automated runtime measurement. This gate is the only way to mark `.codex/rules/responsive-auto-fit.md` and `.codex/rules/layout-overflow-prevention.md` as passed for any HTML deliverable.
...
```bash
node .agents/skills/layout-gate/scripts/layout-gate.cjs <path-to-index.html>
```
...
5. Either per-slide auto-fit JS is installed, OR every slide already fits — if a viewport fails AND auto-fit is missing, advisory points at `.codex/rules/responsive-auto-fit.md` Layer 3
...
```
$ node .agents/skills/layout-gate/scripts/layout-gate.cjs output/foo/index.html
...
Web Developer revises the offending slide using `glass-sm` compaction (per `.codex/rules/presentation-quality.md` Dense Content Compaction), reruns gate, only ships when all PASS.

### Rejection Case (presentation never loads)

```
$ node .agents/skills/layout-gate/scripts/layout-gate.cjs output/broken/index.html
