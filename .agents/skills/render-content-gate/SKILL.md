...
```bash
node .agents/skills/render-content-gate/scripts/render-gate.cjs <path-to-deck.html>
```
...
This gate is the SECOND mandatory evidence file for any HTML deck, run in parallel with the layout gate. The layout gate answers "does it fit?"; the render gate answers "is anything there?". A geometry PASS never substitutes for a render PASS — both `layout-gate-report.json` and `render-gate-report.json` must show `status: "PASS"` before Phase 5 → Phase 6. See `.codex/rules/layout-gate.md`.
...
```
$ node .agents/skills/render-content-gate/scripts/render-gate.cjs output/lesson-20260506-speaking-deck/index.html
...
```
$ node .agents/skills/render-content-gate/scripts/render-gate.cjs output/ai-progress-report/index.html
...
```
$ node .agents/skills/render-content-gate/scripts/render-gate.cjs .agents/skills/render-content-gate/fixtures/blank-slides.html
