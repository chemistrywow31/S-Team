...
(`min-height` / `overflow` required by `.codex/rules/responsive-auto-fit.md`.)
...
This style overrides `.codex/rules/visual-effects-standard.md`'s default 80% effect-coverage requirement, which is calibrated for the tech-mystery / premium-effect default. Minimal modern instead requires that no continuous animations run — the discipline IS the effect.
