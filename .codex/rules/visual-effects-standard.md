...
QA Reviewer must read `.codex/styles/<chosen-style>/tokens.md` Section 8 (`Motion & Effects Policy`) and audit against THAT, not against the defaults in this file. Defaults below remain in force only when no style is declared (which is itself a violation per the Style System mandate).
...
- No style key declared in the Requirements Summary → Violation (per AGENTS.md Style System)
- Style key declared but `.codex/styles/<key>/tokens.md` missing or lacks the required Motion & Effects Policy section → Violation
