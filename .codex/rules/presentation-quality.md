...
All presentations that use selected, user-provided, or edited raster images must also comply with `.codex/rules/presentation-visual-asset-standard.md`.

In short: source raster visuals from existing project assets or user-provided files (the team runs no image APIs — when no raster asset fits, compose the visual from CSS/SVG/icon/typographic elements), keep core slide content as live presentation text, prevent text/image overlap through explicit layout zones, and perform visual QA beyond the automated layout gate.
...
Every HTML presentation must include a responsive auto-fit mechanism. Reference `.codex/rules/responsive-auto-fit.md` for the complete three-layer specification (CSS overflow prevention, reveal.js scale range, per-slide auto-fit JavaScript). Compliance is enforced automatically by `.codex/rules/layout-gate.md` — Web Developer must run the gate and produce a PASS report before any HTML deliverable can be marked complete. Manual viewport-resize self-checks are not accepted as evidence.
...
All HTML presentations must comply with `.codex/rules/visual-effects-standard.md`. Key requirements: 80%+ effect coverage, GPU-only continuous animations, palette-compliant glow colors, transition diversity, `data-auto-animate`, `prefers-reduced-motion`, 60fps target, WCAG 2.3.1 flash safety. The QA Reviewer must verify each item.
...
- A deck with raster images violates `.codex/rules/presentation-visual-asset-standard.md` → Violation
...
- Any visual effects violation defined in `.codex/rules/visual-effects-standard.md` → Violation
...
- HTML presentation missing responsive auto-fit mechanism (see `.codex/rules/responsive-auto-fit.md`) → Violation
