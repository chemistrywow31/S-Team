...
The ladder is keyed to the model actually running the task (dispatch-time override if present, else the agent TOML's `model` + `model_reasoning_effort`): SMALL = `medium` reasoning effort and below, MID = `high`, STRONG = `xhigh`.

- **EC-2.1** A SMALL-tier task that fails its acceptance check gets zero retries at SMALL — escalate to the next tier immediately (`medium` → `high` → `xhigh`), attaching the task, the failed check output, and artifact paths.
- **EC-2.2** A MID-tier (`high`) task that fails gets one retry with a changed approach only; re-running the same approach is forbidden. A second consecutive failure → escalate to STRONG (`xhigh`).
...
- **EC-2.6** When the STRONG tier (`xhigh`) solves an escalated task, write a reusable recipe (preconditions, steps, verification command) to the task worklog before closing; remaining instances are then batch-applied at a lower tier.
...
- **EC-3.2** Acceptance is performed by a fresh-context verifier dispatched via `spawn_agent`, receiving only the acceptance criteria and the artifact paths — never the producer's reasoning, drafts, or history.
...
2. Charter: `AGENTS.md` and this contract
...
- Retry of the same approach after failure at MID tier, or any retry at SMALL tier → Violation
