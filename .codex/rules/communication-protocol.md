...
All task assignments go through the Coordinator. Only the Coordinator dispatches work, via `spawn_agent` from within the main session. Specialists must not assign work to each other, spawn other agents, or re-scope their own dispatch.

### No Peer Messaging

Specialists run in isolated spawned contexts and have no direct-messaging, shared-task-ledger, or team-wide announcement primitives. All inter-agent information flow uses exactly two channels:
...
Parallel groups (Phase 2: Investigative Researcher ∥ Domain Expert; Phase 5: Visual Designer ∥ Web Developer) are concurrent `spawn_agent` dispatches in the same Coordinator turn — parallelism never implies direct peer contact.

### Task Completion Protocol

Every agent ends its run with exactly one EC-1 six-field report to the Coordinator per `.codex/rules/execution-contract.md`: `STATUS` / `CONCLUSIONS` / `EVIDENCE` / `ARTIFACTS` / `RISKS/UNKNOWNS` / `NEXT`. Deliverable locations go in ARTIFACTS; blockers discovered for downstream tasks go in RISKS/UNKNOWNS. There is no separate task-ledger update step — the EC-1 return is the completion signal.
...
Agents work only on what their dispatch scopes. A spawned agent that identifies adjacent work outside its scope fence records it in RISKS/UNKNOWNS or NEXT for the Coordinator to dispatch — it does not perform the work.
