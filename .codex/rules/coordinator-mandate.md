...
The team has exactly one Coordinator, defined at `agents/coordinator.toml` (in the `agents/` root, not a subfolder) and registered in `.codex/config.toml`. The Coordinator runs in the MAIN session: invoking `$boss` makes the current session adopt `coordinator.toml`'s `developer_instructions` as its playbook. The Coordinator must never be spawned as a subagent — a spawned coordinator cannot dispatch further agents and cannot converse with the user.

### Coordinator Does Not Execute

The Coordinator is responsible only for task planning, dispatch, progress tracking, and quality control. It must not perform the execution work of any specialist (research, architecture, content writing, visual design, build, or review). All execution goes through `spawn_agent` dispatch to specialists, each in its own fresh context.

### Flat Architecture

The team uses a flat architecture: the single Coordinator directly manages all nine specialists. Sub-coordinators are prohibited — they add context-relay overhead and degrade information quality in spawn-based dispatch.

### Worktree Isolation for File-Mutating Agents

When dispatching an agent that will modify existing files, the Coordinator must decide whether to isolate the agent's writes in a separate git worktree (or equivalent working copy) and log the decision in `decisions.md`.
