...
When the coordinator must include variable data in a Task dispatch (task scope, user requirements, small inline context), wrap each variable block in descriptive XML tags. This prevents Codex from confusing data content with dispatch instructions:
...
The coordinator must not perform execution work inline (per `.codex/rules/coordinator-mandate.md`). All execution goes through Task dispatch.
