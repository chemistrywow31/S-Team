...
2. `<deliverable-dir>/render-gate-report.json` — content gate, produced by `.agents/skills/render-content-gate/scripts/render-gate.cjs` with top-level `"status": "PASS"`, proving rendered slide content (not just geometry) is present and correct
...
1. Install the canonical Layer 3 auto-fit JS by copying `.agents/skills/layout-gate/scripts/auto-fit-snippet.html` verbatim into `index.html` immediately after `Reveal.initialize()`. Do not write a custom variant.
2. Run `node .agents/skills/layout-gate/scripts/layout-gate.cjs <deliverable>/index.html`.
3. If the gate exits non-zero, revise the offending slides (use `glass-sm` compaction per `.codex/rules/presentation-quality.md` Dense Content Compaction, split overflowing slides, or shrink content) and re-run until it exits zero.
...
- Content density violations (still enforced by `.codex/rules/presentation-quality.md`)
- Text size violations (still enforced by `.codex/rules/layout-overflow-prevention.md`)
- Visual effect quality (still enforced by `.codex/rules/visual-effects-standard.md`)
- PPTX or PDF deliverables (the gate is HTML-only; PPTX/PDF agents must follow `.codex/rules/layout-overflow-prevention.md` text/box constraints and generate at least one full-deck PNG export for QA inspection)
